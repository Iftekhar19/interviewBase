...
export default function ResetPasswordForm() {
  ...
  const onSubmit = async (data) => {
    ...
    message:""
     ...
     await axios.post(`/api/users/forgotpasswordlink`,JSON.stringify({
    ...
    <section className="min-h-screen flex items-center justify-center bg-gradient-to-b from-indigo-50 via-white to-pink-50 px-4">
      ...
      >
        <div className="flex flex-col gap-1 mb-6 items-center">
          ...
          {apiError.isError &&<p className="text-red-500 text-md font-semibold my-2 text-center">{apiError.message}</p>}
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              ...
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 text-gray-600 -translate-y-1/2" size={18} />
                <Input
                  type="text"
            ...
            <Button
              type="submit"
              className="w-full bg-gradient-to-r from-indigo-500 cursor-pointer to-pink-500 text-white font-semibold hover:brightness-105"
              disabled={isSubmitting}
            >
              {isSubmitting ? <><Loader2 className="h-6 w-6 animate-spin"/> Sending</> : "Send Reset Link"}
            </Button>
          </form>
        ) : (
